...
author: "| \\small Juan C. Rocha^1^, Anne-Sophie Crépin^2^\n| \\small\n| \\small ^1^Stockholm
  Resilience Centre, Stockholm University\n| \\small ^2^Beijer Institute of Ecological
...
abstract: Abrupt transitions in ecosystems can be interconnected, raising challenges
  for science and management in identifying sufficient interventions to prevent them
  or recover from undesirable shifts. Here we use principles of network controllability
  to explore how difficult it is to manage coupled regime shifts, that is to recover interconnected systems after they have undergo a bifurcation. We find that coupled
  regime shifts are easier to manage when they share drivers, but can become harder
  to manage if new feedbacks are formed when coupled. Simulation experiments showed
  that both network structure and coupling strength matter in our ability to manage
  interconnected systems. This theoretical observation calls for an empirical assessment
  of cascading regime shifts in ecosystems and warns about our limited ability to
  control cascading effects.
---
...
examples. Regime shifts in ecology are equivalent to the concepts of
bifurcations in mathematics, or phase transitions in physics; where a
regime is an attractor (e.g. a stable equilibrium, a limit cycle, or a
basin of attraction), and the shift marks the bifurcation between one
qualitative type of behaviour and another. Changes in structure and
function of ecosystems can diminish the benefits people get from nature
such as food production, employment opportunities, climate regulation,
or water purification [@Carpenter:2009jr]. For example, the North
Atlantic cod collapse represented substantial economic losses, and
compromised more than 35000 jobs in 400 coastal
communities[@gien2000land]. The weakening or late arrival of the Indian
summer monsoon can compromise food production in one of the most densely
populated regions of the world. How management strategies avoid regime
...
[@Brummitt:2012it; @DSouza:2017hz; @motter2004], social and economic
systems [@Lee:2011p6824, @Brummitt:2017cg; @Anonymous:2015jy], or
diseases [@Watts:2005bq].
...
the commons with potential power asymmetries [@ostrom1990governing].
Classical examples include transnational resource systems where
pollution or exploitation of a resource on one side of the border
impacts its quality on the other side, or downstream
[@young2002institutional; @keys2018megacity]. Second, connected regime
shifts pose the challenge of identifying sufficient and appropriate
...
ability to steer dynamical systems [@Liu:2011p6911]. Early work on the
topic focused on a special type of transition in networks known as
percolation [@motter2002; @motter2004], a continuous phase transition
where a function performed by a network is lost as the giant component
disintegrates (e.g. a blackout on a power grid). Early work
[@motter2002; @motter2004], however, do not take into consideration
discontinuous transitions, which are more common in ecological systems,
and because of hysteresis, they are more challenging for management.

While control theory has been fairly well developed in the realm of
linear systems, many open problems remain for nonlinear ones
[@Cowan:2012fk; @Liu:2015wl; @jiang2019]. Strict global controllability
of nonlinear systems remain elusive, but weaker notions of
controllability can be described [@Liu:2015wl; @Zanudo:2017hw;
@Mochizuki:2013uz; @jiang2019]. For example, open loop control has been
used to steer chaotic systems to desired trajectories in the phase
space. Another example of controllability in ecological food webs shows
limited ability of linear methods when dealing with nonlinear
interactions, and suggest degree ranks as proxy of nodal importance
rather than linear based metrics [@jiang2019].

\<swap the order of the methods, perhaps is more intuitive, and use
subheadings\>

Here we focus on feedback vertex sets as a proxy of structural
controllability [@Liu:2015wl; @Zanudo:2017hw] of regime shifts and their
interactions. First, we use network topology to gain insights on the
controllability of regime shift and the management challenges of their
cascading effects by studying causal networks from the [regime shifts
database](www.regimeshifts.org) [@Biggs:2018hx]. We further test our
intuitions with minimal models of interconnected resources and pollution
recipient systems.
...
\ref{fig:sm_clds}). The data was first reported in [@Rocha:2018gn] and
available at
<https://doi.org/10.6084/m9.figshare.7265096.v1>[.](https://doi.org/10.6084/m9.figshare.7265096.v1.)
...
finding the minimum feedback or arc set is a nondeterministic
polynomial-time hard (NP-hard) problem. Once the system is reduced to a
DAG, linear control methods can be applied, such as finding unmatched
nodes or the Kalman rank condition for linear systems [@Liu:2015wl;
@Liu:2011p6911]. Hence, the set of nodes that need to be controlled to
gain leverage on the behaviour of a nonlinear system are the feedback
vertex set plus the unmatched nodes of the remaining linear system
[@Zanudo:2017hw; @Liu:2015wl].
...
pair-wise combinations (see SM Fig X for a worked example). The
...
[@Rocha:2018gn]. We then compare how structural controllability changes
...
\caption{\textbf{Pollution system simulations} The proportion of nodes recovered by network density for different types of network architecture and coupling strength (A). The proportion of nodes recovered that belongs to the control set (B), and the mean recovery time by proportion of nodes recovered (C). Boxplots and scaterplots summarize N = 9600 networks (100 replicates * 96 experimental settings). Coupling strength in blue for low ranges and red for high ranges \((\delta_{i,j}:{low = 0.01:0.05, high = 0.25:0.5})\) }
\label{fig:pollution}
\end{figure}
...
\caption{\textbf{Resource system simulations} The proportion of nodes recovered by network density for different types of network architecture and coupling strength (A). The proportion of nodes recovered that belongs to the control set (B), and the mean recovery time by proportion of nodes recovered (C). Boxplots and scaterplots summarize N = 9600 networks (100 replicates * 96 experimental settings). Coupling strength in blue for low ranges and red for high ranges \((\delta_{i,j}:{low = 0.01:0.05, high = 0.25:0.5})\) }
\label{fig:resource}
\end{figure}
...
the control set can change over time. Similar vanishing effects have
been reported on the context of identifying the minimum set of variables
to make a system observable, particularly on the context of nonlinear
(non-constant) links between nodes [@letellier2018]. Previous work on
three-node network motifs have shown that coupling strength can increase
observability and controllability up to certain extent, after which it
declines, suggesting a tipping point on the coupling value
[@whalen2015].

Another plausible explanation is that the adjacency matrix ($A_{ij}$) is
non-normal. Networks are non-normal if $AA^T \ne A^TA$, which is the
case in our directed graphs, although matrix asymmetry is not sufficient
for strong non-normality [@asllani2018]. Strong non-normality emerges
when the network is directed (asymmetrical), has low reciprocity, lacks
feedback, and has hierarchical organization [@asllani2018]. While our
networks have feedbacks, we cannot discard the possibility that the lack
of recovery is due to non-normal induced features on the dynamics. The
non-normality of networks increases their return time to equilibrium,
thus reducing their resilience [@asllani2018a]. Symmetries can in fact
induce synchrony on network dynamics [@golubitsky2012], and in have
shown to reduce the controllability of nonlinear systems [@whalen2015]
...
change. However, in our dynamical setting the feedback strength or the
...
combinations (Fig \ref{fig:coup}). Addressing climate alone will not
guarantee avoiding regime shifts [@Anonymous:2015du], successful
strategies are likely to need a sequencing of interventions that address
climate and other drivers in tandem. Our results also confirm that
dealing with climate related variables alone won't be enough to prevent
tipping cascades [@Rocha:2018gn], after all, these variables are a small
...
interventions in the system, nor time bounds or social feasibility. With
limited budget or time to achieve the controllability goals, what would
be an optimal strategy? or a sequence of strategies? From theory we know
that the sequencing of interventions matter for a system to be
controllable [@cornelius2013; @yang2017], but in practice it is unknown
what is that sequence for particular regime shifts or cascading effects.

On our static analysis we assume homogeneity on the coupling strength,
while in reality different systems of the same type (e.g. lakes prone to
eutrophication) will have heterogeneity on the drivers and feedbacks at
play. We break that assumption on our dynamic network models and showed
that, similarly to previous research [@whalen2015], coupling strength
can have differential effects on controllability, in some cases
increasing or decreasing the ability of the manager to recover the full
system.

Another interesting avenue for future research is a case with multiple
managers who have to coordinate to avoid the tipping cascade, adding a
game theoretic layer to the problem. We believe that relaxing any of our
simplifying assumptions to make the problem more realistic would
inevitably make it harder to control. Thus, our results should be
interpreted as an under estimation.
...
other tipping points. Thus the management problem can never be reduced
to a one-dimensional control parameter.
...
\begin{figure}[ht]
\centering
\includegraphics[width = 6in, height = 4in]{Cascading_FigS4}
\caption{\textbf{A worked example} An example of domino effect is depicted on the top panel where the causal diagrams of coral reefs and mangrove transitions appear first individually (left and center) and then combined (right). Our algorithm detects that if coral reefs undergo regime shifts, it will increase coastal erosion, a variable that is in turn a driver of mangrove transitions. An example of a hidden feedback is shown in the lower panel where the causal diagrams of Arctic Sea ice loss and coniferous to deciduous forest are shown individually (left and center) and then coupled together. Our algorithm detects that a new feedback emerge, a circular pathway, related to the increase on frequency of fires and carbon emissions. Source ref 10.}
