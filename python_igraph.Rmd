...
dat <- read_csv2('~/Documents/JUAN/PhD-SRC/RS20+Analysis/RS_CLD_2018.csv')
dat <- dat[,-1]
...
2. Check on Liu's paper if the FVS (I'm assuming is the source node for all FAS edges) is enough to control the network, or if in addition, one needs to find the unmatched nodes as in linear systems.
...
test_all %>% map(., function(x)is.dag(x))
```
...
First convert all networks to bipartite by using the edgelists of the CLDs. Instead of deleting vertices, delete the edges of the feedback arc set first. 
...
dags_all %>% map(., function(x)is.dag(x))
```
...
df_mds <- tibble(
    regime_shifts = names(vertices_control),
